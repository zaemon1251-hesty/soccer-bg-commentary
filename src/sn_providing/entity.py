...
from sn_providing.util import wrap_text
...
class SpottingDataList:
    ...
    def to_json(self, output_file: str):
        json.dump(
            [s.__dict__ for s in self.spottings],
            open(output_file, "w"),
            ensure_ascii=False,
        )
...
class CommentDataList:
    ...
    def filter_by_half_and_time(
        comments: "CommentDataList", half: int, game_time: str, seconds_before: int = 20
    ) -> "CommentDataList":
        """
        ...
        filterd_comments = [
            ...
            if c.half == half
            and c.start_time >= game_time - seconds_before
            and c.start_time < game_time
        ]
    ...
    def get_comment_nearest_time(
        self, game_time: int, thres: float = 15.0, category: str = "0"
    ) -> str:
        diff = float("inf")
        nearest_comment = None
        for comment in self.comments:
            if (
                abs(comment.start_time - game_time) < min(diff, thres)
                and comment.category == category
            ):
    ...
    def to_json(self, output_file: str):
        json.dump(
            [s.__dict__ for s in self.comments],
            open(output_file, "w"),
            ensure_ascii=False,
        )
    ...
    def to_srt(
        self, output_file: str, base_time: float = 0.0, video_end_time: float = 30.0
    ):
        """
...
class VideoData:
    def __init__(
        ...
        assert {"game", "half", "time", "team", "name", "jersey_number"}.issubset(
            set(self.player_df.columns)
        )
        ...
        if label_csv is not None:
            ...
            assert {"game", "half", "time", "label"}.issubset(
                set(self.label_df.columns)
            )

    def get_data(self, game: str, half: int, game_time: int) -> dict[str, str]:
        result_dict = {"players": None, "actions": None}
    ...
    def _preprocess_label_df(label_df: pd.DataFrame) -> pd.DataFrame:
        ...
        label_df["time"] = (
            ...
            .str.split(" - ")
            .str[1]
            .map(VideoData.gametime_to_seconds)
            .astype(float)
        )
...
class ReferenceDoc:
    ...
    def get_list_from_yaml(reference_documents_yaml: str):
        with open(reference_documents_yaml, encoding="utf-8") as file:
            ...
            reference_doc_list: list[ReferenceDoc] = [
                ReferenceDoc(v["id"], v["game"], v["half"], v["time"], v["content"])
                for v in reference_doc_data
            ]
        return reference_doc_list

    @staticmethod
    def get_reference_documents(
        game, half, time, reference_documents: list["ReferenceDoc"]
    ):
        target_dcument = None
        for doc_data in reference_documents:
            if (
                doc_data.game == game
                and doc_data.half == half
                and doc_data.time == time
            ):
    ...
    @staticmethod
    def get_reference_document_entity(
        game, half, time, reference_documents: list["ReferenceDoc"]
    ):
        target_doc = None
        for doc_data in reference_documents:
