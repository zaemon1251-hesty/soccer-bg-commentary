...
def build_query(comments: CommentDataList, max_length: int = 256, **kwargs) -> str:
    """
    検索クエリの内容:
    - 直近のコメント
    ...
    if kwargs.get("players"):
        team_game_str = ", ".join(
            [f"{p['name']} from {p['team']}" for p in kwargs["players"]]
        )
...
def run(args: Arguments):
    ...
    for spotting_data in spotting_data_list.spottings:
        filtered_comment_list = CommentDataList.filter_by_half_and_time(
            comment_data_list, spotting_data.half, spotting_data.game_time
        )

        query_args = {
            "comments": filtered_comment_list,
            "video_data": None,
            "game_metadata": spotting_data_list.game_metadata,
        }

        if video_data is not None:
            video_data_dict = video_data.get_data(
                args.game, spotting_data.half, spotting_data.game_time
            )
